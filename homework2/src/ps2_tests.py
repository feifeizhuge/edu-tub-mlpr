...
def test_em_mog():
    ...
    for _ in range(10):
        ...
        if ((np.abs(np.linalg.det(sigma[0]) - 0.03703) < 0.001 or
            ...
            worked2 = True
        if worked1 and worked2:
            imp.plot_em_solution(X, mu, sigma)
            break

    if not worked1:
        report.append('test_em_mog did not find the correct cluster center.')
    if not worked2:
        report.append('test_em_mog did not find the correct cluster center.')
...
if __name__ == '__main__':
    report = test_kmeans() + test_em_mog() + test_agglo()
    print('=====================================')
    print('Results')
    print('=====================================')
    if len(report) == 0:
