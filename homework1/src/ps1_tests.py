...
def findFirstK_test():
    ...
    distanceMatrix = imp.calcEuclideanDistance(X);
    neighbors = list();
    
    for _,row in enumerate(distanceMatrix):
        _,selectedElements = imp.quickFindFirstK(row, 3);
        neighbors.append(selectedElements)
        
    correct_neighbors = [[0,1,2],[0,1,3],[0,2,3],[1,2,3]];
    
    for i in range(4):
        neighbors[i].sort();
        assert correct_neighbors[i] == neighbors[i], 'Wrong neighbors found. Node:' + str(i) + ' with neighbors:'+str(neighbors[i]);
        
    print 'Tests for find first K elements passed.'
